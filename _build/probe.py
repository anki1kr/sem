...
document.querySelectorAll('svg.sv').forEach(function(svg){
  ...
  // any text bbox clipping a rect it is NOT centred in (a label grazing a neighbour's border)
  texts.forEach(function(t,i){var b=bb[i],cx=b.x+b.width/2,cy=b.y+b.height/2;
    rects.forEach(function(r){var x=r.x.baseVal.value,y=r.y.baseVal.value,w=r.width.baseVal.value,h=r.height.baseVal.value;
      var centred=(cx>x&&cx<x+w&&cy>y&&cy<y+h);
      var ox=Math.min(b.x+b.width,x+w)-Math.max(b.x,x), oy=Math.min(b.y+b.height,y+h)-Math.max(b.y,y);
      if(!centred&&ox>1&&oy>1)res.push(['text-clips-other-rect',cap(svg),t.textContent]);});});
});
...
for key in sys.argv[1:] or list(FOLDERS):
    src = SEM5 / "sessional" / f"{key}-sessional-u1-u2.html"
    if not src.exists():
